...
@section('content')
  <h1 class="mb-6 text-3xl font-bold text-gray-800">
    Edit Materi: <span class="text-xl font-normal text-gray-500">{{ $material->title }}</span>
  </h1>

  <form action="{{ route('mentor.rooms.materials.update', [$room, $material]) }}"
    class="max-w-2xl rounded-lg bg-white p-6 shadow-md md:p-8" enctype="multipart/form-data" method="POST"
    ...
    <div class="mb-4">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="title">Judul Materi</label>
      <input
        class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
        id="title" name="title" type="text" value="{{ old('title', $material->title) }}">
    </div>

    <div class="mb-4">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="description">Deskripsi Singkat</label>
      <textarea
        class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
    ...
    <div class="mb-4">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="type">Tipe Materi</label>
      <select
        class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
    ...
    <div class="mb-4" x-data="{
      ...
      <div :class="{ 'border-indigo-500 bg-indigo-50': isDragging, 'border-gray-300': !isDragging }"
        ...
        @drop.prevent="
         ...
         "
        class="w-full rounded-lg border-2 border-dashed p-4">
        <div @click="$refs.fileInput.click()"
          class="cursor-pointer rounded-lg p-4 text-center text-sm text-gray-600 hover:bg-gray-50">
        ...
        <div class="space-y-2">
          ...
          <template x-if="newFile">
            <label
              class="flex w-full cursor-pointer items-center rounded-lg border-2 border-indigo-500 bg-indigo-50 p-3 shadow-sm">
              <input class="hidden" name="file_selection" type="radio" value="_NEW_FILE_" x-model="selection">

              <template x-if="newFilePreviewURL">
                <img :src="newFilePreviewURL" class="mr-3 h-10 w-10 flex-shrink-0 rounded object-cover">
              </template>
              <template x-if="!newFilePreviewURL">
                <div class="mr-3 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded bg-indigo-100">
              ...
              <button @click.prevent.stop="clearNewFile()" class="ml-3 flex-shrink-0 text-gray-400 hover:text-red-500"
                type="button">
          ...
          @else
            @foreach ($existingFiles as $file)
              ...
              <label
                ...
                class="flex w-full cursor-pointer items-center rounded-lg border p-3 hover:bg-gray-50">
                <input class="hidden" name="file_selection" type="radio" value="{{ $file }}"
                  x-model="selection">

                <div class="mr-3 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded bg-gray-100">
          ...
          <label
            ...
            class="flex w-full cursor-pointer items-center rounded-lg border p-3 hover:bg-gray-50">
            <input class="hidden" name="file_selection" type="radio" value="_NULL_" x-model="selection">
            ...
            <p class="text-sm font-medium text-red-600">Kosongkan (Hapus File)</p>
          </label>
    ...
    <div class="mb-4 rounded-md border bg-gray-50 p-4" x-show="type === 'link'">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="link_url">URL / Link</label>
      <input class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm" id="link_url" name="link_url"
        placeholder="https://contoh.com/materi-anda" type="url"
        value="{{ old('link_url', $material->link_url) }}">
    </div>

    <div class="mb-4 rounded-md border bg-gray-50 p-4" x-show="type === 'text'">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="content">Konten Teks</label>
      <textarea class="w-full rounded-md border border-gray-300 px-3 py-2 shadow-sm" id="content" name="content"
        rows="10">{{ old('content', $material->content) }}</textarea>
    </div>
    <div class="flex justify-end space-x-3">
      <a class="rounded-lg bg-gray-200 px-5 py-2 text-gray-700 hover:bg-gray-300"
        href="{{ route('mentor.rooms.show', $room) }}">
        Batal
      </a>
      <button class="rounded-lg bg-indigo-600 px-5 py-2 text-white transition duration-300 hover:bg-indigo-700"
        type="submit">
        Update Materi
      </button>
    </div>
