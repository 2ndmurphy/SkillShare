...
@section('content')
    <h1 class="text-3xl font-bold text-gray-800 mb-6">
        Edit Materi: <span class="text-xl text-gray-500 font-normal">{{ $material->title }}</span>
    </h1>

    <form action="{{ route('mentor.rooms.materials.update', [$room, $material]) }}" method="POST" enctype="multipart/form-data" 
        ...
        <div class="mb-4">
            ...
            <textarea id="description" name="description" rows="3" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $material->description) }}</textarea>
        </div>
        ...
        <div x-show="type === 'link'" class="mb-4 p-4 bg-gray-50 rounded-md border">
            ...
            <input type="url" id="link_url" name="link_url" value="{{ old('link_url', $material->link_url) }}"
        ...
        <div x-show="type === 'text'" class="mb-4 p-4 bg-gray-50 rounded-md border">
            ...
            <textarea id="content" name="content" rows="10" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm">{{ old('content', $material->content) }}</textarea>
        </div>
        <div class="flex justify-end space-x-3">
            <a href="{{ route('mentor.room.show', $room) }}" class="px-5 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
                Batal
            </a>
            <button type="submit" 
